...
class PublicacionModel
{
    ...
    {
        $query = "SELECT p.*, a.actividad
        FROM publicaciones AS p
        JOIN actividades AS a ON p.id_actividad = a.id 
        WHERE p.id <> $id
        ORDER BY p.id DESC
        LIMIT 4 OFFSET 1;";
    ...
    public function getLast12PublicacionesByActividad($id){
        $query = "SELECT p.*, a.actividad
        FROM publicaciones AS p
        JOIN actividades AS a ON p.id_actividad = a.id 
        WHERE p.id_actividad = $id
        ORDER BY p.id DESC
        LIMIT 12;";
        $resultado = $this->pdo->prepare($query);
        $resultado->execute();
    ...
    public function getPublicacionesByDepIdLimit6($DepId)
    {
        ...
        WHERE p.id_actividad = $DepId AND ultimas_publicaciones.id IS NULL
        ORDER BY p.id DESC
        LIMIT 6;";
    ...
    public function agregarPublicacion($titular, $fecha, $cuerpo, $ruta1, $ruta2, $ruta3, $userId, $actividad) {
        try {
            if (empty($ruta2) && empty($ruta3)) {
                $consulta = "INSERT INTO publicaciones(titular, fecha, cuerpo, imagen1, id_usuario, id_actividad) VALUES ('$titular','$fecha','$cuerpo','$ruta1',$userId,$actividad)";
            } elseif (empty($ruta3) && !empty($ruta2)) {
                $consulta = "INSERT INTO publicaciones(titular, fecha, cuerpo, imagen1, imagen2, id_usuario, id_actividad) VALUES ('$titular','$fecha','$cuerpo','$ruta1','$ruta2',$userId,$actividad)";
            } else {
                $consulta = "INSERT INTO publicaciones(titular, fecha, cuerpo, imagen1, imagen2, imagen3, id_usuario, id_actividad) VALUES ('$titular','$fecha','$cuerpo','$ruta1','$ruta2','$ruta3',$userId,$actividad)";
            }
